...
public class SimpleFilteringRoutingAlgorithm extends AbstractRoutingAlgorithm {

    private final static Logger logger = Logger.getLogger(SimpleFilteringRoutingAlgorithm.class);
    ...
    public SimpleFilteringRoutingAlgorithm() {
        this(0);
    }

    public SimpleFilteringRoutingAlgorithm(int lookahead) {
        this(lookahead, lookahead);
    }

    public SimpleFilteringRoutingAlgorithm(int lookahead, int recomp) {
        this.lookahead = lookahead;
        this.recompute = recomp;
        this.steps = 0;
        logger.info("SimpleRoutingAlgorithm(" + lookahead + "," + recompute + ")");
    }
    ...
    private RoadSegment getMinDelay(List<RoadSegment> neighbors) {
        ...
        neighbors.forEach((neighbor) -> {
            int delay;
            delay = neighbor.getCurrentDelay(Vehicle.class);
            delayLs.add(delay);
